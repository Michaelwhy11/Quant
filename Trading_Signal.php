...
//Should get an array like array('date' => moving average)
function Get_EMA($price_array_small, $period)
{
    $dates = array_keys($price_array_small);
    $prices = array_values($price_array_small);
    $priceCount = count($prices);
    ...
    $N = 2 / ($period + 1);
    $ema = array();
    $ema[$dates[0]] = $prices[0];
    for($i = 1; $i < $priceCount; $i++)
    {
        $ema[$dates[$i]] = $N * $prices[$i] + (1 - $N) * $ema[$dates[$i - 1]];
    }
    return $ema;
}
//This function find the trend of the stocks. If diff go above dea(which should be under dea when bearish, 
//since dea is the moving average of diff), it states a buying signal because the price is going up, vice versa.
function MACD($ema_26, $ema_12, $period = 9)
{
    ...
    $dea = array();
    $macd = array();
    $diff[$dates[0]] = $ema_12[$dates[0]] - $ema_26[$dates[0]];
    $dea[$dates[0]] = $diff[$dates[0]];
    $macd[$dates[0]] = 2 * ($diff[$dates[0]] - $dea[$dates[0]]);
    $N = 2 / ($period + 1);
    for($i = 1; $i < $counts; $i++)
    {
        $d = $dates[$i];
        $diff[$d] = $ema_12[$d] - $ema_26[$d];
        $dea[$d] = $N * $diff[$d] + (1 - $N) * $diff[$dates[$i - 1]];
